...
任务详情把会话入口放在它所属的生命周期事实旁：顶部“当前对话”展示当前执行的阶段、状态和 Codex App 入口；每条有来源对话的持久活动链接到自己的 thread，因此多轮审查分别保留各自入口。当前有效的请求决定通过当前执行已提交的活动精确识别，并提供直接回复操作；已解决的问题只作为历史活动保留。看板不提供回复输入，也不生成临时活动。
