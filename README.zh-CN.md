...
更新窗口会展示当前版本、最新稳定版、最后检查时间，并分别显示四个托管 Skills 和一个托管 Codex Hook 的状态。自动检查只更新这些状态，安装仍需用户明确确认。确认后，窗口会安装指定版本、重启本地服务、同步并验证全部五个随包资源，再验证运行版本并记录成功。对应的终端命令是：
...
`PROJECT.md` 是项目选择和任务执行读取的唯一当前产品事实。项目注册完成后，Agent 直接用普通文件工具局部修改这份本地文档，再发送只包含版本与哈希的轻量通知，不重新传输完整文档。Codrive 会重新读取并验证文件，把决定摘要写入只追加的事件历史，替换已经失效的任务选择并重新规划。Codrive 只接受当前持久化状态 schema，不转换早期产品上下文数据。完整契约见 [产品事实生命周期](./docs/architecture/product-facts.md)。
