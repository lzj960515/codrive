...
Codrive 负责确定性的部分：持久化生命周期状态、执行尝试、Codex 任务 ID、全局并发上限，以及同一个仓库一次只合入一个任务。Codex 负责需要判断的部分：选择任务、实现、审查、返工、Git 操作、冲突解决、合入和产品验收。

任务选择不是预先固定的依赖图。每次出现空闲位置时，一个临时 Codex 任务都会读取最新的产品、任务和仓库事实，并且可以一次选中多个适合立即开始的任务。Codrive 验证选择结果后，为每个任务分别创建顶层 Codex 开发任务和独立工作树。默认最多同时运行四个任务；不同任务可以并行开发和审查，同一个仓库仍然一次只合入一个任务。

项目注册、新增任务、阶段报告、Codex 回合完成或失败、用户控制和服务启动，都会立即触发下一步判断。并发上限也是判断条件的一部分，因此并发数发生变化后，之前的选择结果会自动失效并重新判断。Codrive 启动时会把早期的单任务默认配置自动升级为四任务并发。

Codrive 每分钟还会进行一次范围很小的自动恢复检查：发送正在等待对话空闲的任务消息，恢复长时间没有结果的 AI 工作；如果运行中的项目仍有未开始任务，但当前完全没有 Codex 工作，则重新启动任务选择。App Server 仍报告为 `inProgress` 的回合会保留当前 attempt 并续租；只有确认不存在或已经终止的回合才会创建替代 attempt。服务启动时会先完成恢复，再开放控制命令，避免用户重试与启动恢复竞争。
...
重试和取消表达不同的生命周期语义。任务或项目执行失败且仍保留待执行动作时，重试会创建新的 attempt；等待用户输入的任务在原对话中继续同一个 attempt；取消会永久终止任务或项目。
