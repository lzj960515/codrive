...
Codrive 每分钟还会进行一次范围很小的自动恢复检查：发送正在等待对话空闲的任务消息，并恢复长时间没有结果的 AI 工作。运行中的项目完全没有 Codex 工作时，恢复器只启动尚未评估的规划版本；已经保存的选择、等待决定或阻塞结果不会重复调用 AI。App Server 仍报告为 `inProgress` 的回合会保留当前 attempt 并续租；只有确认不存在或已经终止的回合才会创建替代 attempt。服务启动时会先完成恢复，再开放控制命令，避免用户重试与启动恢复竞争。
