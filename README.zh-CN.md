...
看板先通过带认证的 HTTP 读取权威快照，再由一条带认证的 WebSocket 连接接收后续变化。实时事件包含协议版本、连接内序号，以及最小的 project、task、settings 或 system 作用域。断线重连或 sequence 缺口只重新同步相关 HTTP 投影，同时保留当前项目、已打开的任务详情、滚动位置、更新弹窗、焦点和尚未提交的输入。
...
HTTP 始终负责看板、产品、任务、设置和系统状态的初始权威读取，`/api/commands` 继续作为幂等写入边界。`/api/live` WebSocket 只通知带作用域的变化，不承载命令，也不替代权威快照。协议与恢复模型见[实时同步架构](./docs/architecture/live-sync.md)。
...
HTTP API 与 WebSocket 都只监听 `127.0.0.1`，并使用同一个随机访问令牌。自动 Codex 任务拥有完整本机访问权限，可以连续修改、测试、提交、合入和清理代码，无需等待终端审批。请只注册你信任的仓库和产品指令。
