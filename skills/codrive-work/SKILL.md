...
3. 从当前对话、产品契约和已有决定中明确新需求的结果、范围和完成标准；只澄清会改变结果的未决事项。
...
5. 拟定新增或调整后的任务粒度、前置结果或迁移批次时，读取[任务拆分方法](references/task-slicing.md)。向用户展示产品文档变化和任务计划变化；已有明确确认覆盖同一份变化时继续执行，尚未确认的变化在写入前取得确认。
