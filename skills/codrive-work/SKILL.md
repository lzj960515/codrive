...
6. 新增普通任务先按[任务文档模板](references/task-document.md)在主项目仓库的 `docs/tasks/` 写好并读回文档，再使用 `add` 登记相对路径；创建阶段目标使用 `milestone-create`，目标可以先没有任务。修改未开始任务的登记信息或已确认里程碑定义使用 `$codrive-control`；修改已有任务文档正文直接编辑原文件。只有真实产品事实改变时才先编辑 `PROJECT.md`，并在新增工作或任务修改中携带产品变更元数据。
