...
3. 从当前对话、产品契约、里程碑和已有决定还原任务产生的背景、具体问题、目标结果，以及已确认做法与选择理由，并确认授权。已确认目标内的调查、必要漏项和实现选择直接推进；新增产品能力、改变既有结果或扩大阶段范围时，带着事实、影响、选项与推荐向用户请求决定。
...
6. 新增普通任务先按[任务文档模板](references/task-document.md)在主项目仓库的 `docs/tasks/` 写好并读回文档，核对背景、问题、目标、已确认做法及理由和验收能否让接手者理解为何这样做，再使用 `add` 登记相对路径；创建阶段目标使用 `milestone-create`，目标可以先没有任务。修改未开始任务的登记信息或已确认里程碑定义使用 `$codrive-control`；修改已有任务文档正文直接编辑原文件。只有真实产品事实改变时才先编辑 `PROJECT.md`，并在新增工作或任务修改中携带产品变更元数据。
...
普通未开始任务的名称、里程碑归属或文档路径需要调整时，读取 `$codrive-control` 并使用其 `task-update` 命令。已有文档路径的任务需要调整正文时，在主项目目录按[任务文档模板](references/task-document.md)编辑原文件，并读回核对背景、问题、目标、做法理由和验收；下一轮规划、执行和审查会读取当前内容。没有文档路径的历史任务修改结果边界或验收标准时，沿用 `description` 和 `acceptanceCriteria` 的定义修改，也可为未开始任务写好文档后设置路径，转为文档模式。登记信息与长期产品事实同时变化时，在命令中携带 `productDocumentChange`，让 Codrive 用一个规划修订接受两项事实。命令成功后重新读取任务，确认返回的登记信息、任务状态和新规划均来自服务端持久化结果。
