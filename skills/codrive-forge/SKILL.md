...
2. 从当前对话和已有资料提取目标用户与场景、现有做法及问题、产品目标、已确认的关键做法与理由、范围和完成标准；只澄清仍会改变结果的缺口。
...
5. 用户确认后，按[任务文档模板](../codrive-work/references/task-document.md)为每项新任务在项目仓库的 `docs/tasks/` 写好文档，读回正文，核对背景、问题、目标、做法理由和验收之间的关系，再核实路径。确保当前目录具备可供 Codrive 创建工作树的本地 Git 基线；需要时在当前目录初始化仓库、默认分支和初始提交。
