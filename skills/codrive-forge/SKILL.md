...
5. 用户确认后，按[任务文档模板](../codrive-work/references/task-document.md)为每项新任务在项目仓库的 `docs/tasks/` 写好文档，读回正文并核实路径。确保当前目录具备可供 Codrive 创建工作树的本地 Git 基线；需要时在当前目录初始化仓库、默认分支和初始提交。
