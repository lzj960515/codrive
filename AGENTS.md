...
The detached upgrade worker installs the exact package, restarts its service, synchronizes all managed Skills and Hooks from that package, verifies their status, and verifies the running version before recording success. Later user removal remains an explicit repair action instead of being silently reversed on every restart.
