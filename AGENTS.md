...
Every task turn enters through `$codrive-task`. After reading the current task definition, acceptance criteria, requested action, complete activity history, and repository rules, that Skill selects and loads other available Skills whose descriptions match the actual work for the current stage. Tasks without another matching Skill continue through the ordinary `$codrive-task` workflow. Lifecycle-owned requirements such as independent `$code-review` remain explicit dispatch guidance.

`PROJECT.md` is the only current product-facts source for project selection and task turns. `Project.productFacts` stores the accepted file revision and digest; decision summaries stay in append-only lifecycle events. Agents edit the local file with normal filesystem tools, then send a lightweight change notification carrying the prior revision/digest and the edited-file digest. `WorkflowEngine` validates the file, supersedes stale project selection, advances planning, and reconciles scheduling. Ordinary unstarted backlog task definitions change through `task.update_definition` with optimistic concurrency; the same serialized command can accept an edited `PROJECT.md` when the task revision changes product facts. Task JSON remains internal persisted lifecycle state. `project.add_work` requires an independent decision summary and permits unchanged product facts; real document changes remain optional coordinated input. State schema v5 is the current persisted contract. Startup converts supported older data under the state lock before App Server or Recovery. Migration preserves task identities and exact work bindings, leaves old tasks independent, and replaces temporary planning with pending persistent selection. Runtime accepts only the new data model and current API. See [Product facts lifecycle](./docs/architecture/product-facts.md).
