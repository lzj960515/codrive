...
The same in-memory bridge keeps a last-seen window for the exact task execution. A service restart starts a fresh window instead of guessing that a quiet turn has stopped. After ten signal-free minutes, Codrive checks the saved thread and turn through App Server once per minute: a running turn starts a new ten-minute window, a completed turn enters the normal report path, and only an unambiguously interrupted or failed turn is eligible to resume. App Server may report a persisted thread as not loaded after restart; its exact terminal turn remains authoritative when no turn is active. Missing, unreadable, contradictory, superseded, paused, or capacity-blocked work stays unchanged and is checked again later; Codrive does not create a persisted presence state.
